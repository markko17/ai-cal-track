...
// Validate required Firebase config values (fail fast if missing)
...
if (missingKeys.length > 0) {
  throw new Error(`[Firebase] Missing config keys: ${missingKeys.join(', ')}. Restart Expo server with: npx expo start -c`);
}
